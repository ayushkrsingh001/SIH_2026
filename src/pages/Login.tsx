    ...
    <div className="min-h-screen flex items-center justify-center p-gutter font-body text-on-surface bg-cream relative">
      ...
      >
        ...
        <div className="w-full md:w-1/2 bg-secondary-container p-lg flex-col justify-center items-center text-center relative overflow-hidden hidden md:flex">
          ...
          <h2 className="font-headline text-headline-md text-on-secondary-container mb-4 z-10">{t('auth.welcomeTitle')}</h2>
          <p className="font-body text-body-lg text-on-secondary-container max-w-sm z-10">{t('auth.welcomeDesc')}</p>
        </div>
        ...
        <div className="w-full md:w-1/2 p-lg md:p-xl flex flex-col justify-center">
          ...
          <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
            <div>
              <label className="block font-body text-label-md text-on-surface-variant mb-2" htmlFor="login-email">{t('auth.emailLabel')}</label>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="block font-body text-label-md text-on-surface-variant" htmlFor="login-password">{t('auth.passwordLabel')}</label>
                <Link to="/forgot-password" className="font-body text-label-md text-primary hover:text-primary-container transition-colors">{t('auth.forgotPassword')}</Link>
              </div>
              <div className="relative">
            ...
            >
              {loading ? <span className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : (
                <>{t('auth.logInBtn')} <span className="material-symbols-outlined">arrow_forward</span></>
              )}
            </button>
          </form>
          ...
          <p className="mt-8 text-center font-body text-body-md text-on-surface-variant">
            {t('auth.newToRightsQuest')} <Link to="/signup" className="text-primary font-semibold hover:underline">{t('auth.createAccountLink')}</Link>
          </p>
        </div>
      </motion.div>
    </div>
